 ...
 * Definitions
 */
define( 'PASSWORDLESS_LOGIN_VERSION', '1.0.9' );
	...
	if ( $account && ! is_wp_error( $sent_link ) ) {
		...
		if ( is_plugin_active( 'profile-builder-pro/index.php' ) || is_plugin_active( 'profile-builder/index.php' ) || is_plugin_active( 'profile-builder-hobbyist/index.php' ) ) {
			...
			if ( $wppb_general_options !== false ) {
				...
				} elseif ( $wppb_general_options['loginWith'] == 'username' ) {
...
function wpa_send_link( $email_account = false, $nonce = false ) {
	...
	if ( is_wp_error( $valid_email ) ) {
		...
		$message    = apply_filters( 'wpa_email_message', sprintf( __( 'Hello ! <br><br>Login at %1$s by visiting this url: <a href="%2$s" target="_blank">%3$s</a>', 'passwordless-login' ), $blog_name, esc_url( $unique_url ), esc_url( $unique_url ) ), $unique_url, $valid_email );
...
function wpa_autologin_via_url() {
	if ( isset( $_GET['token'] ) && isset( $_GET['uid'] ) && isset( $_GET['nonce'] ) ) {
		...
		if ( ! $wp_hasher->CheckPassword( $token . $hash_meta_expiration, $hash_meta ) || $hash_meta_expiration < $time || ! wp_verify_nonce( $nonce, 'wpa_passwordless_login_request' ) ) {
			...
			exit;
		} elseif ( defined( 'PROFILE_BUILDER_VERSION' ) && $wppb_generalSettings != 'not_found' && ! empty( $wppb_generalSettings['adminApproval'] ) && $wppb_generalSettings['adminApproval'] == 'yes' && wp_get_object_terms( $uid, 'user_status' ) ) {// admin approval compatibility
...
require_once 'inc/wpa.class.notices.php';
